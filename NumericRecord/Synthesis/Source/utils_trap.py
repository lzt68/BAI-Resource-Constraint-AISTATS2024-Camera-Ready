...
def Balanced_Trap_HalfConsumption_Fix32_MultiR_HL(
    K: int, L: int = 2, rlow=0.1, rmid=0.8, rhigh=0.9, dlow: float = 0.1, dhigh: float = 0.9
...
def Balanced_Trap_HalfConsumption_Fix32_MultiR_Mixture(
    K: int, L: int = 2, rlow=0.1, rmid=0.8, rhigh=0.9, dlow: float = 0.1, dhigh: float = 0.9
) -> tuple[np.ndarray, np.ndarray]:
    """Arms with higher reward consume less resource, but the mean reward and consumption only have 3 values
...
def Balanced_Trap_HalfConsumption_Fix32_MultiR_HH(
    K: int, L: int = 2, rlow=0.1, rmid=0.8, rhigh=0.9, dlow: float = 0.1, dhigh: float = 0.9
) -> tuple[np.ndarray, np.ndarray]:
